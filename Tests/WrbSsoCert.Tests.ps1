...
Describe ('WrbSsoCert.help topic') {
    It 'should have one' {
        Init
        GivenModuleImported
        ThenHelpTopic 'about_WrbSsoCert' -Exists
    }
}

Describe ('WrbSsoCert.command verbs') {
...
Describe ('WrbSsoCert.command help topics') {
    It 'should have a help topic for each command' {
        Init
        GivenModuleImported
        foreach( $cmd in (Get-Command -Module 'WrbSsoCert' -CommandType Function,Cmdlet,Filter))
        {
